...
public class BOJ_14500_김영민 {
	...
	static void otherBlock(int x, int y) {
		int temp = 0;
		// ㅗ
		if (x < N && y + 2 < M && x - 1 >= 1 && y >= 1) {
			temp = map[x][y] + map[x][y + 1] + map[x - 1][y + 1] + map[x][y + 2];
			result = Math.max(result, temp);
		}
		// ㅏ
		if (x + 2 < N && y + 1 < M && x >= 1 && y >= 1) {
			temp = map[x][y] + map[x + 1][y] + map[x + 1][y + 1] + map[x + 2][y];
			result = Math.max(result, temp);
		}
		// ㅜ
		if (x + 1 < N && y + 2 < M && x >= 1 && y >= 1) {
			temp = map[x][y] + map[x][y + 1] + map[x + 1][y + 1] + map[x][y + 2];
			result = Math.max(result, temp);
		}
		// ㅓ
		if (x + 1 < N && y + 1 < M && x - 1 >= 1 && y >= 1) {
			temp = map[x][y] + map[x][y + 1] + map[x - 1][y + 1] + map[x + 1][y + 1];
			result = Math.max(result, temp);
		}
	}
}
